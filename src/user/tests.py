...
class PublicUserAPITests(TestCase):
    ...
    def test_create_valid_user_success(self):
        """"Testing cresting a user with a valid payload is succesfull"""
        payload = {
    ...
    def test_create_token_invalid_credentials(self):
        ...
        create_user(email='test@davis.com', password='pass1234')
        payload = {'email':'test@davis.com', 'password':'wrongpassword'}
        res = self.client.post(TOKEN_URL, payload)
    ...
    def test_retrieve_user_unauthorised(self):
        """Test that authentication is required for a given user"""
        res = self.client.get(ME_URL)
        self.assertEqual(res.status_code, status.HTTP_401_UNAUTHORIZED)
#
#
class PrivateUserAPITests(TestCase):
    """Tests API requiests that require authentication  """

    def setUp(self):
        self.user = create_user(email='test@davis.com', password='testpass', name='Test User')
        self.client = APIClient()
